...
public class ShapeList extends ArrayList<GenericShapeWrapper<?>> {
     ...
     */
    public boolean add(Client client, ShapeData shape) {
        if (shape instanceof PointData) {
            return super.add(new PointWrapper(client, (PointData) shape));
        } else if (shape instanceof TextData) {
            return super.add(new TextWrapper(client, (TextData) shape));
        } else if (shape instanceof RectangleData) {
            return super.add(new RectangleWrapper(client, (RectangleData) shape));
        } else if (shape instanceof MaskData) {
            return super.add(new MaskWrapper(client, (MaskData) shape));
        } else if (shape instanceof EllipseData) {
            return super.add(new EllipseWrapper(client, (EllipseData) shape));
        } else if (shape instanceof LineData) {
            return super.add(new LineWrapper(client, (LineData) shape));
        } else if (shape instanceof PolylineData) {
            return super.add(new PolylineWrapper(client, (PolylineData) shape));
        } else if (shape instanceof PolygonData) {
            return super.add(new PolygonWrapper(client, (PolygonData) shape));
        } else {
            return super.add(new ShapeWrapper(client, shape));
        }
    }
